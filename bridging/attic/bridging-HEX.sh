...
if [ $runa -eq 1 ]; then
    ...
    boxl=`tail -n 1 $idir/$mypath/operators.dat | awk '{print sqrt($7*$7+$8*$8)}'` #get absolute length since hexagonal box
...
if [ $runc -eq 1 ]; then
    ...
    cmd="sed  -e '/celllengths/s/=.*/=\ $lxnew\ $lynew/g'
              ...
              -e '/TimeStepDT/s/=.*/=\ 1.0/g' 
              -e '/ReadInputFields/s/=.*/=\ yes/g'
