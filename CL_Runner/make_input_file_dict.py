...
toolpath = op.read()
chainpath =toolpath+'/geninput_v1.1/'
...
from Chains import Input_Builder_SCFT,Component_Statistics_Counter,\
...
def make_input_SCFT(PHASE,REPLACE_DICT,WDIR):
...
#    space_group =  REPLACE_lIST[10]
    ...
    #new additions
    if 'graft_density' in var_list:
        graft_density = REPLACE_DICT['graft_density']
    else:
        graft_density = 1
    
    if 'gauss_width' in var_list:
        gauss_width = REPLACE_DICT['gauss_width']
    else:
        gauss_width = 0.2/Nref
    
    if 'chain_density' in var_list:
        chain_density = REPLACE_DICT['chain_density']
    else:
        chain_density = 1.0
    
    if 'calcstruct_bool' in var_list:
        calcstruct_bool = REPLACE_DICT['calcstruct_bool']
    else:
        calcstruct_bool = 'True'
    ...
    #C 
    statistics_interactions_add = [[f'compressibility_invzetaN = {invzeta}'],\
    ...
    gauss_smear_text = parameter_species(gauss_width,n_species)
    f = open(input_file_path,'w+')    
    
    # f,inputfileversion,nummodel,modeltype,n_species,\
    ...
    Input_Builder_SCFT(f,inputfileversion,nummodel,modeltype,n_species,\
                     kuhn_length_text,gauss_smear_text,chain_label,n_sidearm_types,\
                     models_add,chain_text_list,chiN_list,interation_add,\
                     ...
                     block_num,dt,force_scale_text,stress_scale,force_tol,stress_tol,\
                     variablecell_bool,density_history_bool,field_history_bool,\
                     density_chain_bool,format_fields_bool,volfrac_chain,chain_density,cellscale,
