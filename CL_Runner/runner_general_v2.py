...
from make_input_file_dict import make_input_SCFT
import numpy as np
import os
import pdb
import shutil
...
def domain_size_extractor_noncubic(content,d):
    list_of_cell = list(filter(lambda x: 'a_1' in x, content))
    # print('High Dim')
    r = list_of_cell[0]
    Box_initial = abs(float(r[r.find("(")+1:r.find(",")]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-graft', '--inv_graft_density', action='store',nargs='+', default=1,help='Grafting Density',type=int)
    parser.add_argument('-kuhn', '--kuhn_length', action='store',nargs='+', default=[1],help='Kuhn Length',type=float)
    parser.add_argument('-gwidth', '--gauss_width', action='store',nargs='+', default=[0.2],help='gaussian width',type=float)
    parser.add_argument('-cdensity', '--chain_density', action='store', default=20,help='chain density',type=float)
    parser.add_argument( '--invzeta', action='store', default=0.1,help='Compressibility',type=float)
            ...
            for n in range(0,countlist[1]):
                ...
                for o in range(0,countlist[2]):
                    ...
                    for p in range(0,countlist[3]):
                        ...
                        REPLACE_DICT['stress_scale'] = args.stressscale
                        REPLACE_DICT['force_scale'] = args.forcescale
                        REPLACE_DICT['chain_list'] = chain_list
                        REPLACE_DICT['npw'] = args.number_of_planewaves[q]
                        REPLACE_DICT['Nref'] = args.reference_length
                        ...
                        REPLACE_DICT['kuhn_length'] = args.kuhn_length
                        REPLACE_DICT['nThreads'] = args.NThreads[q]
                        REPLACE_DICT['graft_density'] = args.inv_graft_density
                        REPLACE_DICT['gauss_width'] = args.gauss_width
                        REPLACE_DICT['chain_density'] = args.chain_density
                        REPLACE_DICT['cellscale'] = args.initial_box_size[q]
                        REPLACE_DICT['invzeta'] = args.invzeta
                        if args.NThreads[q]!=1:
                        ...
                        make_input_SCFT(Phase,REPLACE_DICT,WDIR)
