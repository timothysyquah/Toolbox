...
def extractData(filehandler,column,warmup):
    ...
    if warmup > 0:
        return Data1[:warmup],Data1[warmup:]
    else:
...
def doStats(warmupdata,Data,doGraphs=False,doWriteStdout=False,graphFilenameStub=''):
    ...
    autocor=cor[cor.size//2:] # // is integer division; both cor.size and 2 are integers so the result will be integer type
    ...
    if doWriteStdout == True:
        ...
        sys.stdout.write( "  - Equilibrated samples    = {}\n".format(nsamples))
        sys.stdout.write( "  - Correlation time        = {}\n".format(kappa))
        sys.stdout.write( "  - Effective # samples     = {}\n".format(nsamples/kappa))
        sys.stdout.write( "  - Reduced-bias variance   = {}\n".format(unbiasedvar))
        ...
        sys.stdout.write( "  - S.D. (unbiased, biased) = {} {}\n".format(np.sqrt(unbiasedvar),np.std(Data,ddof=0))) # ddof is correction to 1/N...using ddof=1 returns regular reduced-bias estimator)
        sys.stdout.write( "  - Skewness                = {}\n".format(skew))
        sys.stdout.write( "  - Kurtosis                = {}\n".format(kurtosis))
  ...
  if args.col != None:
    for i in args.col:
      ...
      if args.autowarmup:
          ...
          if not args.quiet:
            print("Auto warmup detection with MSER-5 => {0}".format(nwarmup))
      else:
    ...
    for i in args.observable:
      ...
      if args.autowarmup:
          ...
          if not args.quiet:
            print("Auto warmup detection with MSER-5 => ".format(nwarmup))
      else:
