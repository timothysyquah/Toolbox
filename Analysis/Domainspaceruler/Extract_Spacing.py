    ...
    if len(df.index)>1:
        ...
        if plotdebug:
            ...
            for j in range(3):
                ...
                plt.tight_layout()
                plt.savefig(f'/home/tquah/Figures/nbb{str(temparray[0][2]+1)}.pdf',dpi = 300)

        meanD0 = Predict_D0(array,Method)
...
exportarray = np.vstack(arraylist)
rearange = np.argsort(exportarray[:,2])



np.savetxt(Export_Path,exportarray[rearange,:])