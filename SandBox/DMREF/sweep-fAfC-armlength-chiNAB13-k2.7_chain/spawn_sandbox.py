...
if __name__=="__main__":
    ...
    dS = 0.01
    dt = np.array([5e-2,1e-2,5e-3,1e-3,5e-4,1e-4])
    stress_scale = np.array([1,0.5,0.1,0.05,0.005,0.001])
    ...
    for i in range(0,len(dt),1):
        for j in range(0,len(stress_scale),1):
            for k in range(0,len(PhaseList),1):
                ...
                WDIR=f'dt{dt[i]:0.3f}/stressmobil{stress_scale[j]:0.2f}/{PhaseList[k]}Phase/'
                ...
                sidearm_length_1 = Nsc
                sidearm_length_2 = Nsc
                sidearm_length_3 = Nsc
                ...
                REPLACE_lIST = [fieldpath,chain_list,chiN,d,chain_label,dS,dt[i],stress_scale[j],\
