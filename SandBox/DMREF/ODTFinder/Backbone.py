...
#need to populate
IDIR = os.getcwd()
DISpath = os.path.join(IDIR,initial_field_dir,'DIS_fields.in')
...
for m in range(0,len(chi),1):
    ...
    for n in range(0,len(armlength),1):
        ...
        N_initial = int(np.around(chiN_initial/(chi[m]*(armlength[n]+1))))
        if N_initial%2:
            N_initial+=1
        
        
        Ssweep = np.arange(N_initial,0,-2,dtype = int)
        ...
        while i!=len(Ssweep):
            ...
            if i==0:
                jobstatus=1                
                field_DIR = os.path.join(IDIR,initial_field_dir,'LAM_fields.in')
            ...
            if os.path.exists(WDIR):
                ...
                replace_list_DIS=[DISpath,chain_list,chiN,npw,\
                      50,d,jobname,paramlist[0],\
                      paramlist[1],paramlist[2],paramlist[3]]

                jobstatus=0
                os.makedirs(WDIR)
                LAM_path = os.path.join(WDIR,'LAM')
                DIS_path = os.path.join(WDIR,'DIS')

                os.makedirs(LAM_path)
                os.makedirs(DIS_path)

                make_input(phase,replace_list,LAM_path)
                make_input('DIS',replace_list_DIS,DIS_path)

                make_submit(submitfile,phase,jobname,LAM_path)
                make_submit(submitfile,'DIS',jobname,DIS_path)

                jobid = qsub_py(submitfile,LAM_path,IDIR)
                qsub_py(submitfile,DIS_path,IDIR)

                past_field_dir.append(LAM_path)
