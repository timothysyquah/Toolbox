...
###############################################################################
#alter params
Ssweep = np.array(list(np.arange(130,160+1e-6,2))+\
                  list(np.arange(164,200+1e-6,4))+\
                  list(np.arange(206,240+1e-6,6))+\
                  list(np.arange(242,280+1e-6,8))+\
                  list(np.arange(290,350+1e-6,10))+\
                  list(np.arange(360,500+1e-6,20))+\
                  list(np.arange(540,700+1e-6,40)))
dS = 1.0  #countour length
DT = 0.002  #time length
lambda_force = [1.0,1.0]
lambda_stress = 0.001
    ...
    for n in range(0,len(armlength),1):
        ...
        while i!=len(Ssweep):
            ...
            if i==0:
                ...
                field_DIR = os.path.join(armpath,past_field_dir[i-1])
